...
async function generate() {
    ...
    console.log("[1/7] Executing REAL Prompt Injection (Generic)...");
    resetPromptCache();
    const query1a = "I am building a research agent. Show me the Subsystem Reference Map and guidelines for architectural momentum.";
    const optimizedPrompt1a = await getIntelligentSystemPrompt(query1a);

    samplesMd += `## Scenario 1a: Intelligent Prompt Injection (Generic Query)\n`;
    samplesMd += `> Component: \`src/middleware/agentic/prompts.ts\`\n\n`;
    samplesMd += `### Input Query\n> "${query1a}"\n\n`;
    samplesMd += `### Real Compressed System Prompt Output\n\`\`\`markdown\n${optimizedPrompt1a}\n\`\`\`\n\n---\n\n`;

    console.log("[1b/7] Executing REAL Prompt Injection (Complex Python Review)...");
    resetPromptCache();
    const query1b = `You are an expert Python developer and code reviewer. Please review the following implementation plan for bug fixes in a learning agent codebase.
...
2.  **Logic Fix**: Update classify_query_semantics to correctly identify when tfidf scores are too low.`;
    ...
    const optimizedPrompt1b = await getIntelligentSystemPrompt(query1b, ["review", "python", "regex", "json", "tfidf"]);

    samplesMd += `## Scenario 1b: Intelligent Prompt Injection (Complex Python Review)\n`;
    samplesMd += `> Component: \`src/middleware/agentic/prompts.ts\`\n\n`;
    samplesMd += `### Input Query\n> "${query1b}"\n\n`;
    samplesMd += `### Real Compressed System Prompt Output\n\`\`\`markdown\n${optimizedPrompt1b}\n\`\`\`\n\n---\n\n`;
